...
@JsonDeserialize(
    using = RetryPaymentException.Deserializer.class
)
public final class RetryPaymentException extends Exception {
  private final Value value;

  private int statusCode;

  private RetryPaymentException(Value value, int statusCode) {
    this.value = value;
  ...
  public static RetryPaymentException other(Object unknownValue, int statusCode) {
    return new RetryPaymentException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "RetryPaymentException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<RetryPaymentException> {
    @Override
    public RetryPaymentException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new RetryPaymentException(value, statusCode);
    }
  }
}
