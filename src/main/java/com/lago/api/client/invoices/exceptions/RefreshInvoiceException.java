package com.lago.api.client.invoices.exceptions;
...
@JsonDeserialize(
    using = RefreshInvoiceException.Deserializer.class
)
public final class RefreshInvoiceException extends Exception {
  private final Value value;

  private int statusCode;

  private RefreshInvoiceException(Value value, int statusCode) {
    this.value = value;
  ...
  public static RefreshInvoiceException other(Object unknownValue, int statusCode) {
    return new RefreshInvoiceException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "RefreshInvoiceException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<RefreshInvoiceException> {
    @Override
    public RefreshInvoiceException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new RefreshInvoiceException(value, statusCode);
    }
  }
}
