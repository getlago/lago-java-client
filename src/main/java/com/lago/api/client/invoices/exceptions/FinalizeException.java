...
@JsonDeserialize(
    using = FinalizeException.Deserializer.class
)
public final class FinalizeException extends Exception {
  private final Value value;

  private int statusCode;

  private FinalizeException(Value value, int statusCode) {
    this.value = value;
  ...
  public static FinalizeException other(Object unknownValue, int statusCode) {
    return new FinalizeException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "FinalizeException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<FinalizeException> {
    @Override
    public FinalizeException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new FinalizeException(value, statusCode);
    }
  }
}
