...
@JsonDeserialize(
    using = FindMetricGroupsException.Deserializer.class
)
public final class FindMetricGroupsException extends Exception {
  private final Value value;

  private int statusCode;

  private FindMetricGroupsException(Value value, int statusCode) {
    this.value = value;
  ...
  public static FindMetricGroupsException other(Object unknownValue, int statusCode) {
    return new FindMetricGroupsException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "FindMetricGroupsException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<FindMetricGroupsException> {
    @Override
    public FindMetricGroupsException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new FindMetricGroupsException(value, statusCode);
    }
  }
}
