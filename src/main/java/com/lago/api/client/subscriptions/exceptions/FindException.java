package com.lago.api.client.subscriptions.exceptions;
...
@JsonDeserialize(
    using = FindException.Deserializer.class
)
public final class FindException extends Exception {
  private final Value value;

  private int statusCode;

  private FindException(Value value, int statusCode) {
    this.value = value;
  ...
  public static FindException other(Object unknownValue, int statusCode) {
    return new FindException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "FindException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<FindException> {
    @Override
    public FindException deserialize(JsonParser p, DeserializationContext ctx) throws IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new FindException(value, statusCode);
    }
  }
}
