...
@JsonDeserialize(
    using = ApplyException.Deserializer.class
)
public final class ApplyException extends Exception {
  ...
  private ApplyException(Value value, int statusCode) {
    this.value = value;
  ...
  public static ApplyException other(Object unknownValue, int statusCode) {
    return new ApplyException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "ApplyException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<ApplyException> {
    @Override
    public ApplyException deserialize(JsonParser p, DeserializationContext ctx) throws IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new ApplyException(value, statusCode);
    }
  }
}
