...
public class GetCustomerPortalUrl200ResponseCustomer {
  ...
  * @throws IOException if the JSON Object is invalid with respect to GetCustomerPortalUrl200ResponseCustomer
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!GetCustomerPortalUrl200ResponseCustomer.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!GetCustomerPortalUrl200ResponseCustomer.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GetCustomerPortalUrl200ResponseCustomer` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
