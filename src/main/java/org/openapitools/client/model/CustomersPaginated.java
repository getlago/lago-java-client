...
public class CustomersPaginated {
  ...
  private List<CustomerObject> customers = new ArrayList<>();
  ...
  public CustomersPaginated customers(List<CustomerObject> customers) {
    
    this.customers = customers;
    return this;
  }

  public CustomersPaginated addCustomersItem(CustomerObject customersItem) {
  ...
  **/
  ...
  public void setCustomers(List<CustomerObject> customers) {
    this.customers = customers;
  }
  ...
  * @throws IOException if the JSON Object is invalid with respect to CustomersPaginated
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CustomersPaginated.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CustomersPaginated.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CustomersPaginated` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraycustomers.size(); i++) {
        CustomerObject.validateJsonObject(jsonArraycustomers.get(i).getAsJsonObject());
      };
