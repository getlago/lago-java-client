...
public class BillableMetricsPaginated {
  ...
  * @throws IOException if the JSON Object is invalid with respect to BillableMetricsPaginated
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!BillableMetricsPaginated.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!BillableMetricsPaginated.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BillableMetricsPaginated` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraybillableMetrics.size(); i++) {
        BillableMetricObject.validateJsonObject(jsonArraybillableMetrics.get(i).getAsJsonObject());
      };
