...
public class AppliedCouponsPaginated {
  ...
  * @throws IOException if the JSON Object is invalid with respect to AppliedCouponsPaginated
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!AppliedCouponsPaginated.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!AppliedCouponsPaginated.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AppliedCouponsPaginated` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayappliedCoupons.size(); i++) {
        AppliedCouponObjectExtended.validateJsonObject(jsonArrayappliedCoupons.get(i).getAsJsonObject());
      };
