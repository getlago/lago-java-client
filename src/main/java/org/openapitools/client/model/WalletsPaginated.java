...
public class WalletsPaginated {
  ...
  * @throws IOException if the JSON Object is invalid with respect to WalletsPaginated
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!WalletsPaginated.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!WalletsPaginated.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `WalletsPaginated` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraywallets.size(); i++) {
        WalletObject.validateJsonObject(jsonArraywallets.get(i).getAsJsonObject());
      };
