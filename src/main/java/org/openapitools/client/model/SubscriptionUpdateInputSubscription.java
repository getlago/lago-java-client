...
public class SubscriptionUpdateInputSubscription {
  ...
  * @throws IOException if the JSON Object is invalid with respect to SubscriptionUpdateInputSubscription
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!SubscriptionUpdateInputSubscription.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!SubscriptionUpdateInputSubscription.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SubscriptionUpdateInputSubscription` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
